...
export default function () {
  ...
  function getCode(email: string | number | undefined) {
    const getCodeAx = () =>
      apiClient.get(`/recruit/user/sendEmailCode/${email}`);
    ...
    watch(data, () => {
      ...
      if (res.code == 1007) {
        ...
        showAlert("验证码发送失败,请联系招新群管理员", "error");
      }
    });
  }

  // 提交报名表
  function sentStuInfo(stuInform: FormData, isVisibleRef: Ref<boolean>) {
    const sentStuInfoAx = () =>
      apiClient.post("/recruit/user/register", stuInform, {
